...
var calendar = function(b)
{
	...
	p.applnk( J.path() + '../style/date.css' ); J(p._main).acls('cal_panel');
...
J.exend( calendar.prototype, {
	...
	createbody: function(d,m)
	{
		...
		J(chead).stcs({ width: '154px', height: '30px' }).html(txt); J(m).apch(chead);
		...
		txt = '<table width="100%" cellspacing="0" cellpadding="0" border="0">' +
			  ...
			  '<div id="foot"><span id="t1">今天</span><span id="t2">' + new Date().toLocaleDateString() + '</span></div>';
		
		var cbody = J(d).crte('div'); J(cbody).stcs('width','154px').html(txt); J(m).apch(cbody);
		...
		J('t1',d).hover(function(){
			...
			if( J.calendar.type == '-')
				...
				J(J.calendar.inu).val( m + '/' + t + '/' + y ); J.calendar._p.hide( this, J.calendar._p );
		});
	},
